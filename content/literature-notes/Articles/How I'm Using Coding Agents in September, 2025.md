...
## Metadata
- Author: [[fsck.com]]
- Full Title: How I'm Using Coding Agents in September, 2025
...
- Since I last wrote at the beginning of the summer, my methodology for using AI coding assistants has evolved a bit. This is a point-in-time writeup of a flow that's been pretty effective for me.
  I'm still primarily using Claude Code. ([View Highlight](https://read.readwise.io/read/01k7yv349h84nq1h3g0hhkvwbg))
- First up, this is my [CLAUDE.md](https://raw.githubusercontent.com/obra/dotfiles/6e088092406cf1e3cc78d146a5247e934912f6f8/.claude/CLAUDE.md) as of this writing. It encodes a bunch of process documentation and rules that do a pretty good job keeping Claude on track.
  When I want to start a new task on an existing project, I try to always use a git worktree to isolate that work from other tasks. This is increasingly important for me, because I find myself frequently running 3-4 parallel projects on a single codebase. ([View Highlight](https://read.readwise.io/read/01k7yv4yh1mz4rmzk0w14qc878))
- Once I've got claude code running, I use my "brainstorming" prompt:
  ...
  Once you believe you understand what we're doing, stop and describe the design to me, in sections of maybe 200-300 words at a time, asking after each section whether it looks right so far. ([View Highlight](https://read.readwise.io/read/01k7yv5rs57az54t3681ahc8m9))
- That last bit is particuarly critical. I find that AI models are expecially prone to handing me walls of text when they think they're "done". And I'm prone to just tuning out a bit and thinknig "it's probably fine" when confronted with a wall of text written by an agent. By telling Claude to limit its output to a couple hundred words at a time, I'm more likely to actually read and engage. ([View Highlight](https://read.readwise.io/read/01k7yv61w7srrc3w8sszdgy6qc))
...
- The next step is the planning process. Here's the planning prompt I've been using:
  ...
  please write out this plan, in full detail, into docs/plans/ ([View Highlight](https://read.readwise.io/read/01k7yv8fmzn769xkjdm8s3vbr3))
- This results in a plan that breaks everything down into tiny little steps with clear instructions and tightly packed context for each step. That means that at execution time, I usually don't need to provide tight step by step oversight.
  Next up, I open a *new tab* or window in the same working directory and fire up another copy of claude. I tell it something like `Please read docs/plans/this-task-plan.md and <whatever we named the design doc>. Let me know if you have questions.` ([View Highlight](https://read.readwise.io/read/01k7yv9hvwckqtf1m4gkcjmcby))
- It will usually say that the plan is very well crafted. Sometimes it'll point out mistakes or inconsistencies. Putting on my PM hat, I'll then turn around and ask the "architect" session to clarify or update the planning doc.
  ...
  The implementer will chug along. ([View Highlight](https://read.readwise.io/read/01k7yva0r1j2mr9265stt0v5he))
- When it's done, I'll flip back to the "architect" session and tell it `The implementer says it's done tasks 1-3. Please check the work carefully.`
  I'll play PM again, copying and pasting reviews and Q&A between the two sessions. Once the architect signs off, I'll tell the implementer to update the planning doc with its current state. ([View Highlight](https://read.readwise.io/read/01k7yvaesz6b4fkjjwnm497dew))
- And then, I don't `*/compact*`. Instead I `*/clear*` the implementer and start the conversation over. Telling it that it's starting with task 4.
  When it's done with the next chunk of work, I flip back to the architect. I typically double-`ESC` to reset the architect to a previous checkpoint and tell it to review up to the now-current checkpoint. This reduces context bloat for the architect and gets it to look at again without any biases from the previous implementation. ([View Highlight](https://read.readwise.io/read/01k7yvavx92bggxyq4kj4ba6mw))
- (I have friends who, instead of using multiple sessions, swear that just asking the implementer to look at their most recent work `with fresh eyes` is good enough. And indeed, using that magic phrase seems to be pretty powerful. I still think that having two different actors is better.) ([View Highlight](https://read.readwise.io/read/01k7yvb39d0swjzvr7wy0hj1v1))
...
- The only problem with tools like this is that our robot buddies are quite credulous. If you paste in a list of instructions for how to update a codebase, Claude's just going to take you at your word and make the changes, even if what you're asking for is crazy and wrong.
  ...
  Anything we should skip, tell me about now. ([View Highlight](https://read.readwise.io/read/01k7yvd4sn2fwk4tvvpyz605w3))
- CodeRabbit "reviewers" typically get a 'Strong hire' review, but it's not unheard of for Claude to report that the reviewer "seems quite technically adept, but didn't take the time to understand our project and made a number of suggestions that are wrong. No hire." ([View Highlight](https://read.readwise.io/read/01k7yvdp3rgxt94ytndq3eg24h))
